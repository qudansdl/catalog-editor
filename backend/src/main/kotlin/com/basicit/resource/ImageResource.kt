...
class ImageResource(private val imageService: ImageService) {
  ...
  @GetMapping("/{id}")
  fun getImageById(@PathVariable(value = "id") imageId: String): ResponseEntity<Image> =
  ...
  @PutMapping("/{id}")
  fun updateImageById(
    @PathVariable(value = "id") imageId: String,
  ...
  @DeleteMapping("/{id}")
  fun deleteTask(@PathVariable(value = "id") imageId: String): ResponseEntity<Void> =
